...
// Checks if db has existing blocks, if not- creates Genesis Block
func InitBlockChain() *BlockChain {
	...
	err = db.Update(func(txn *badger.Txn) error {
		// No last hash i.e. no blocks in blockchain yet
		if _, err := txn.Get([]byte("lh")); err == badger.ErrKeyNotFound {
			...
			return err
		} else { // Blockchain in db exists, returns last hash and creates local copy of blockchain
			item, err := txn.Get([]byte("lh"))
			Handle(err)
			err = item.Value(func(val []byte) error {
				lastHash = val
